...
import numpy as np
import random
from pc_color_seg.handler.predict import Predict
from pc_color_seg.handler.train_model import TrainModel
    ...
    def main(self):
        while True:
            ...
            if not choice:
                ...
                output_dir_path = input("Output Dir Path: ").strip('"').strip()
                plot_dir_path = input("Plot Dir Path: ").strip('"').strip() or None
                real_marker_pair_length = float(input("Marker Pair Length (mm): "))
                real_marker_pair_center_distance = float(input("Marker Pair Center Distance (mm): "))
                headless = input("Headless (y/n): ").lower().strip() == "y"
